...
```powershell
powershell -ExecutionPolicy Bypass -File .\deploy\build-image.ps1 -Version 3.1.7
```

脚本会生成：

- `deploy/twinkle-image-3.1.7.tar`
- `deploy/twinkle-image-3.1.7.tar.sha256`
...
```text
twinkle-image-3.1.7.tar
twinkle-image-3.1.7.tar.sha256
docker-compose.yaml
.env
...
sha256sum -c twinkle-image-3.1.7.tar.sha256
docker load -i twinkle-image-3.1.7.tar
docker image inspect twinkle-image:3.1.7 --format '{{.Id}} {{index .Config.Labels "org.opencontainers.image.version"}}'
```

`docker load` 导入后，Compose 的 `image:` 应填写镜像标签
`twinkle-image:3.1.7`，不是 tar 文件名。
